...
def train_cnn_autoencoder(x_train, x_test, encoding_dim=ENCODING_DIM_CNN):
    ...
    autoencoder.summary()
    weight_path_autoencoder = os.path.join(WEIGHTS_PATH, 'autoencoder_weights_cnn_more_filter.h5')
    autoencoder.save_weights(weight_path_autoencoder)

    # create the encoder model
    encoder = Model(input_training, encoded)
    weight_path_encoder = os.path.join(WEIGHTS_PATH, 'autoencoder_weights_encoder_cnn_more_filter.h5')
