...
ENCODING_DIM_CNN = 10
LOAD_WEIGHTS = False
AUTOENCODER_NAME = "cnn_many_filter"
SHUFFLE = True
EPOCHS = 50
BATCH_SIZE = 64
...
def train_cnn_autoencoder(x_train, x_test, encoding_dim=ENCODING_DIM_CNN):
    ...
    weight_path_autoencoder = os.path.join(WEIGHTS_PATH, "CNN_DEEPEST_weights_e50_dim10_ba64" + now.strftime("_%Y-%m-%d-%H:%M:%S") + ".h5")
    autoencoder.save(weight_path_autoencoder)

    # create the encoder model
    encoder = Model(input_training, encoded)
    weight_path_encoder = os.path.join(WEIGHTS_PATH, "CNN_DEEPEST_weights_encoder_e50_dim10_ba64" + now.strftime("_%Y-%m-%d-%H:%M:%S") + ".h5")
